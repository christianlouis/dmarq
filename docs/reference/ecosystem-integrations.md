...
| --- | --- | --- | --- | --- | --- | --- |
| DNS providers: Cloudflare, Route 53, Akamai Edge DNS/FastDNS, Hetzner, Linode, Google Cloud DNS, Azure DNS, DigitalOcean, PowerDNS, registrar DNS, hosting-panel DNS | DMARC, SPF, DKIM, MTA-STS, TLS-RPT, BIMI, and DANE fixes usually live in DNS. | Detect authoritative provider, import zones, inspect records, create safe change plans. | Provider API token limited to zone read first; DNS edit scope only when the user enables repair. Route 53 should prefer role assumption/external ID. Akamai Edge DNS should use EdgeGrid. | Zones, record sets, TTLs, DNS snapshots, provider metadata. | Approval-only TXT/CNAME create/update first; no hidden deletions, wildcard changes, or ambiguous SPF rewrites. | [#423](https://github.com/christianlouis/dmarq/issues/423), [#380](https://github.com/christianlouis/dmarq/issues/380), [#381](https://github.com/christianlouis/dmarq/issues/381) |
